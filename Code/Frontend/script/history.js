...
const drawChart = function (data, sensorName, timestamps) {
  document.querySelector('.js-chart').innerHTML = `<canvas id="myChart" width="300" height="100"></canvas>`;
  ...
  let config = {
    ...
    data: {
      ...
      datasets: [
        {
          // label: sensorName, //label that we added at the top
...
const showSensors = function (jsonObject) {
  console.log(jsonObject);
  let htmlString = `<option class="js-sensor-option" value=""></option>`;
...
const init = function () {
  ...
  console.log(html_poweroffButton);
  getHistory(109);
  getSensors();
  listenToSocket();
  toggleNav();
